...
\usepackage{graphicx}
\graphicspath{{./static/}}                        % set graphics path to figs folder, ie now all file imports can be referenced relative to figs
...
\usepackage[outputdir=build]{minted}                             % for beautiful listings
...
\listofsymbols{symbols}				%.tex file met daarin de lijst van symbolen
...
\campus{brugeseng}                       % Define your campus and language (append "eng" to load the English template)
                                            % campuses: denayer, geel, gent, groept, brugge (denayereng, geeleng, ghenteng, groupteng, brugeseng)
\title{Modeling object localization uncertainty for robot grasping}
\subtitle{} % invullen moest het nodig zijn, gwn typen in {}
\forenameA{Thijs}
\surnameA{Alens}
...
\promotorA[Supervisor(s)]{Prof Dr Mathias Verbeke\\Dr Ir Laurens Devos} %for English use Supervisor(s)
...
\input{chapters/chapter2_literature-study.tex}
\input{chapters/chapter3_dataset.tex}
\input{chapters/chapter4_models.tex}
\input{chapters/chapter5_safety-mechanisms.tex}
