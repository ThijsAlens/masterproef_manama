...
Luggage gets sorted and autonomously transported across an airport, autonomous vacuum cleaners are being used daily, medicines are being produced automatically with extreme cleanliness, etc.
...
However, \gls{ai} systems often struggle when faced with new, unknown situations, leading to unreliable predictions.
Systems should therefore be able to express their uncertainty about their predictions, enabling safety mechanisms to prevent it from taking actions based on uncertain predictions.
...
One of the issues with these systems is that they will always give you an answer, even when they are unsure about it.
...
One example of why a system could be unsure about its output is when it needs to infer something from data that is not seen (properly) during training.
Most training data is gathered in controlled environments, where all conditions are optimal.
However, when used in real-world scenarios with conditions different from the training environment, like different lighting conditions because of shadows or reflections, some dust on the camera lens causing some distortion, someone turning the lights off, etc. the system could become a lot worese at its task.
...
\begin{itemize}
    ...
    \item Create a dataset that simulates real-world conditions, including various challenges such as occlusions, lighting variations, unexpected object placements, etc.
