...
\cref{fig:pixel-vs-real-world} visually shows the difference between training a model to output pixel coordinates and training a model to output real world coordinates and its effects on the uncertaitny.

\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{static/pixel-vs-real-world.png}
    \caption{The left side shows a model trained to output pixel coordinates, which does not account for the uncertainty of the translation from pixel space to real world coordinates. The right side shows a model trained to output real world coordinates, which can learn to account for the uncertainty of the translation from pixel space to real world coordinates.}
    \label{fig:pixel-vs-real-world}
\end{figure}

\section{Quantifying uncertainty} \label{ch:literature-study_section:quantifying-uncertainty}
Models who can handle / quantify the uncertainties mentioned in~\ref{ch:literature-study_section:uncertainty-in-ai} are therefore sought-after in safety-critical applications like robot arm grasping.
