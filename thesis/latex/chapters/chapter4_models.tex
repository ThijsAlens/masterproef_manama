...
Generally, the architecture consists of a few convolutional layers followed by a single fully connected layers.
The first convolutional layer has 3 or 4 input channels, depending on the input data to include the depth channel or not.
The fully connected layer should have 4 output channels, two for the predicted x and y coordinates, and two for the predicted uncertainty of those coordinates.

As an example, say we have an architecture with three convolutional layers, the first one with 8 output channels, the second one with 32 output channels, and the third one with 64 output channels.
After the convolutional layers, the output is flattened and fed to a fully connected layer with 4 output channels.
...
The ResNet architecture we used for our experiments is a ResNet-18, which is quite a small version of ResNet with, as the name implies, 18 residual blocks.
...
The main difference is, of course, the addition of dropout layers after each convolutional layer.